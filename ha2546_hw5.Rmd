...
confusionMatrix(data = svml_pred2,
...
From confusion matrices above, we can observe that SVM with linear kernel has training error rate of `r round((1-0.8425)*100, 2)`% and SVM with radial kernel has training error rate of `r round((1-0.84)*100, 2)`% These values are very close to each other. 
...
Also, the summary of *resamples()* function show the mean accuracy of 0.8323957 (cross validation error = `r round((1-0.8323957)*100, 2)`%) for SVM with linear kernel and the mean accuracy of of 0.8298957 (cross validation error rate = `r round((1-0.8298957)*100, 2)`%) for SVM with radial kernel. So, if we want to select one of these models, I think the one with linear kernel is a better choice because the model is less complicated while the cross validated is not that different (less than 1 percent).
